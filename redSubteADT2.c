...
typedef struct infoMolineteCDT
{
	char * fecha;
	char * horaInicio;
	char * horaFinal;
...
infoMolineteADT newInfoMolinete(){
	return calloc(1, sizeof(infoMolineteCDT));
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
redSubteADT agregarEstacion(infoADT e, redSubteADT red){					// Agrega la estación e a la red de subte
	red -> first = agregarEstacionRec(e, red -> first);				// Se hará de forma recursiva
	return red;

}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////static void sumarPasajerosDia();				//////////////////////HACER ESTA FUNCION////////////////////////
///////////////////////////////////////////////////////////////en agregarDato///////////////////////////////////////////////

static estacionADT buscarID(int id, estacionADT est){
	if (est == NULL)
	{
		return est;
	}
	...
	return buscarID(id, est -> next);

}


static lineaADT agregarDatoLinea(infoMolineteADT nuevaInfo, lineaADT l){
	if (l == NULL)
	{
		return l;
	}
	estacionADT aux = newEstacion();
	aux = buscarID(nuevaInfo -> id, l -> first);
	if (aux == NULL)
	{
		l -> next = agregarDatoLinea(nuevaInfo, l -> next);
		return l;
	}
	l -> total += nuevaInfo -> cantPas;

	aux -> cant_pas = nuevaInfo -> cantPas;
	if (nuevaInfo -> cantPas > l -> max_pas)
	{
		l -> max_pas = nuevaInfo -> cantPas;
		l -> est = aux -> nombre;
	}
	l -> next = agregarDatoLinea(nuevaInfo, l -> next);
	return l;
...
void agregarDato(infoMolineteADT nuevaInfo,redSubteADT r){
	r -> total_pasajeros += nuevaInfo -> cantPas;
	r -> first = agregarDatoLinea(nuevaInfo, r -> first);
	//sumarPasajerosDia();
}
...
void pasajerosLinea(redSubteADT h){							// Imprime la cantidad de pasajeros de cada línea
...
static void porcRec(lineaADT l, long int total, FILE * query3){								// Recorre cada línea indicando el porcentaje de
	...
	float porc = l -> total * 100.0 / total;
	fprintf(query3,"%c, %g\n", l-> nombre, porc);
...
infoMolineteADT guardarDatoMolinete(infoMolineteADT nuevaEst, char * texto){			//Guarda los datos de cada linea en una estructura info
	char * dato = strtok(texto, ",");
	nuevaEst -> fecha = dato;
	dato = strtok(NULL, ",");
	nuevaEst -> horaInicio = dato;
	dato = strtok(NULL, ",");
	nuevaEst -> horaFinal = dato;
	dato = strtok(NULL, ",");
	nuevaEst -> id = atoi(dato);
	dato = strtok(NULL, ",");
	nuevaEst -> cantPas = atoi(dato);
	return nuevaEst;
}
...
////////////////////////////////////////////////////////////////////////////////////////////	TESTEO
void toBegin(redSubteADT r){
	r -> current = r -> first;
	r -> first -> current = r -> first -> first;
}
...
void printNext(redSubteADT r){
	...
	while(r -> current != NULL){
		printf("%c\n", r -> current -> nombre);
		r -> current -> current = r -> current -> first;

		while(r -> current -> current != NULL){
			printf("\t%s\n", r -> current -> current -> nombre);
			r -> current -> current = r -> current -> current -> next;
		}	
